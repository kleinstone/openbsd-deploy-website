...
echo "======================================================================"
echo "[$LOG_TIME] 🚀 Starting Atomic Deployment for: ${SITE_NAME}"
echo "======================================================================"
...
if [ -d "${SYMLINK_PATH}" ] && [ ! -L "${SYMLINK_PATH}" ]; then
    echo "[$LOG_TIME] ⚠️  Notice: ${SYMLINK_PATH} is a real directory."
    echo "[$LOG_TIME] 📦 Backing it up to 'public_legacy_backup' to transition to atomic symlinks..."
    mv "${SYMLINK_PATH}" "${BASE_DIR}/public_legacy_backup"
fi
# -----------------------------------

# 5. Copy new files to the isolated release directory
echo "[$LOG_TIME] 📂 Copying release to ${NEW_RELEASE_DIR}..."
cp -Rf "${SOURCE_DIR}/." "${NEW_RELEASE_DIR}/"

# 5b. Link Shared Resources
echo "[$LOG_TIME] 🔗 Linking persistent shared resources..."
...
for item in $SHARED_ITEMS; do
    if [ -e "${SHARED_DIR}/${item}" ]; then
        ...
        echo "  -> ⚠️  Notice: '${item}' not found in shared/. Skipping symlink."
    fi
done

# 6. Fix permissions BEFORE going live
echo "[$LOG_TIME] 🔒 Fixing permissions on shared and release directories..."
...
if [ -x "${SCRIPT_DIR}/fix-web-perms.ksh" ]; then
    ...
    echo "[$LOG_TIME] ⚠️  Warning: fix-web-perms.ksh not found in PATH or ${SCRIPT_DIR}."
    # Optional: Put fallback chmod/chown commands here
fi

# 7. The Atomic Switch
echo "[$LOG_TIME] 🔄 Switching live traffic to the new release..."
...
echo "[$LOG_TIME] 🧹 Cleaning up old releases..."
...
echo "======================================================================"
echo "[$LOG_TIME] ✨ Successfully deployed ${SITE_NAME}!"
echo "======================================================================"
